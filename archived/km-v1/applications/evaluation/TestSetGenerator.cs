...
public sealed partial class TestSetGenerator : EvaluationEngine
{
    ...
    private KernelFunction Translate => _evaluatorKernel.CreateFunctionFromPrompt(GetSKPrompt("Transmutation", "Translate"),
    ...
    private KernelFunction QuestionAnswerGeneration => _evaluatorKernel.CreateFunctionFromPrompt(GetSKPrompt("SyntheticData", "QuestionAnswer"),
        new OpenAIPromptExecutionSettings
        {
            Temperature = 1e-8f,
            Seed = 0,
    ...
    private KernelFunction KeyPhraseExtraction => _evaluatorKernel.CreateFunctionFromPrompt(GetSKPrompt("Extraction", "Keyphrase"),
        new OpenAIPromptExecutionSettings
        {
            Temperature = 1e-8f,
            Seed = 0,
    ...
    private KernelFunction SeedQuestionGeneration => _evaluatorKernel.CreateFunctionFromPrompt(GetSKPrompt("SyntheticData", "SeedQuestion"),
        new OpenAIPromptExecutionSettings
        {
            Temperature = 1e-8f,
            Seed = 0
    ...
    private KernelFunction ReasoningQuestionGeneration => _evaluatorKernel.CreateFunctionFromPrompt(GetSKPrompt("SyntheticData", "ReasoningQuestion"),
        new OpenAIPromptExecutionSettings
        {
            Temperature = 1e-8f,
            Seed = 0
    ...
    private KernelFunction MultiContextQuestionGeneration => _evaluatorKernel.CreateFunctionFromPrompt(GetSKPrompt("SyntheticData", "MultiContextQuestion"),
        new OpenAIPromptExecutionSettings
        {
            Temperature = 1e-8f,
            Seed = 0
    ...
    private KernelFunction ConditioningQuestionGeneration => _evaluatorKernel.CreateFunctionFromPrompt(GetSKPrompt("SyntheticData", "ConditionalQuestion"),
        new OpenAIPromptExecutionSettings
        {
            Temperature = 1e-8f,
            Seed = 0
    ...
    internal TestSetGenerator(
        ...
        IMemoryDb memoryDb)
    {
        _evaluatorKernel = evaluationKernel.Clone();
        _translationKernel = (translationKernel ?? evaluationKernel).Clone();
        _memory = memoryDb;
    }
        ...
        Distribution? distribution = null)
    {
        ...
        await foreach (var record in _memory.GetListAsync(index, limit: int.MaxValue).ConfigureAwait(false))
        {
        ...
        {
            var partitionRecords = await _memory.GetListAsync(index,
            ...
            var questions =
                GetSimpleQuestionTestSetsAsync(nodes.Take(simpleCount), language, retryCount)
                    .Concat(GetReasoningTestSetsAsync(nodes.Skip(simpleCount).Take(reasoningCount), language, retryCount))
                    .Concat(GetMultiContextTestSetsAsync(nodes.Skip(simpleCount + reasoningCount).Take(multiContextCount), language, retryCount))
                    .Concat(GetConditioningTestSetsAsync(nodes.Skip(simpleCount + reasoningCount + multiContextCount).Take(conditioningCount), language, retryCount));
        ...
        int retryCount = 3)
    {
        foreach (var partition in nodes)
        {
            ...
            var question = await GetMultiContextQuestionAsync(seedQuestionContext,
                    alternativeContext,
                    seedQuestion,
                    language,
                    retryCount)
            ...
            var groundTruth = await GetQuestionAnswerAsync(seedQuestionContext + " " + alternativeContext,
                    question,
                    language,
                    retryCount)
                .ConfigureAwait(false);
    ...
    private Task<string> GetMultiContextQuestionAsync(
        string context1,
        string context2,
        string seedQuestion,
        string language = null!,
        int retryCount = 3)
    {
        return Try(retryCount,
            async remainingTry =>
            {
                var question = await MultiContextQuestionGeneration.InvokeAsync(_evaluatorKernel,
                        new KernelArguments
                        {
                            { "question", seedQuestion },
                            { "context1", context1 },
                ...
                {
                    question = await Translate.InvokeAsync(_evaluatorKernel,
                            new KernelArguments
                            {
        ...
        int retryCount = 3)
    {
        foreach (var partition in nodes)
        {
            ...
            var question = await GetReasoningQuestionAsync(nodeText,
                    seedQuestion,
                    language,
                    retryCount)
                .ConfigureAwait(false);

            var groundTruth = await GetQuestionAnswerAsync(nodeText,
                    question,
                    language,
                    retryCount)
                .ConfigureAwait(false);
    ...
    private Task<string> GetReasoningQuestionAsync(
        string context,
        string seedQuestion,
        string language = null!,
        int retryCount = 3)
    {
        return Try(retryCount,
            async remainingTry =>
            {
                var question = await ReasoningQuestionGeneration.InvokeAsync(_evaluatorKernel,
                        new KernelArguments
                        {
                            { "question", seedQuestion },
                            { "context", context }
        ...
        int retryCount = 3)
    {
        foreach (var partition in nodes)
        {
            ...
            var question = await GetConditioningQuestionAsync(nodeText,
                    seedQuestion,
                    language,
                    retryCount)
                .ConfigureAwait(false);
    ...
    private Task<string> GetConditioningQuestionAsync(
        string context,
        string seedQuestion,
        string language = null!,
        int retryCount = 3)
    {
        return Try(retryCount,
            async remainingTry =>
            {
                var question = await ConditioningQuestionGeneration.InvokeAsync(_evaluatorKernel,
                        new KernelArguments
                        {
                            { "question", seedQuestion },
                            { "context", context }
        ...
        int retryCount = 3)
    {
        return Try(retryCount,
            async remainingTry =>
            {
                ...
                var seedQuestion = await SeedQuestionGeneration.InvokeAsync(_evaluatorKernel,
                        new KernelArguments
                        {
                ...
                if (!string.IsNullOrEmpty(language))
                {
                    seedQuestion = await Translate.InvokeAsync(_evaluatorKernel,
                            new KernelArguments
                            {
    ...
    {
        var keyPhrases = await Try(retryCount,
                async remainingTry =>
                {
                    var generatedKeyPhrases = await KeyPhraseExtraction.InvokeAsync(_evaluatorKernel,
                            new KernelArguments
                            {
                                { "input", context }
                            })
        ...
        return Shuffle(keyPhrases!.Statements!);
    }


    private Task<QuestionAnswer> GetQuestionAnswerAsync(
        string context,
        string question,
        string language = null!,
        int retryCount = 3)
    {
        return Try(retryCount,
            async remainingTry =>
            {
                var generatedAnswer = await QuestionAnswerGeneration.InvokeAsync(_evaluatorKernel,
                        new KernelArguments
                        {
                            { "context", context },
                            { "question", question }
                ...
                if (!string.IsNullOrEmpty(language))
                {
                    generatedAnswer = await Translate.InvokeAsync(_evaluatorKernel,
                            new KernelArguments
                            {
                                { "input", answer.Answer },
                                { "translate_to", language }

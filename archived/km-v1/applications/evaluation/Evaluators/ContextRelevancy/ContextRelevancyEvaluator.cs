...
internal sealed class ContextRelevancyEvaluator : EvaluationEngine
{
    ...
    private KernelFunction EvaluateContext => _kernel.CreateFunctionFromPrompt(GetSKPrompt("Evaluation", "ContextPrecision"),
    ...
    {
        var relevancy = await Try(3,
                async tryCount =>
                {
                    var verification = await EvaluateContext.InvokeAsync(_kernel,
                            new KernelArguments
                            {

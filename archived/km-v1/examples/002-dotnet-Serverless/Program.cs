...
public static class Program
{
    ...
    private static async Task StoreText()
    {
        ...
        var docId = await s_memory.ImportTextAsync("In physics, mass–energy equivalence is the relationship between mass and energy " + "in a system's rest frame, where the two quantities differ only by a multiplicative " + "constant and the units of measurement. The principle is described by the physicist " + "Albert Einstein's formula: E = m*c^2");
    ...
    private static async Task StoreFile()
    {
        ...
        var docId = await s_memory.ImportDocumentAsync("file1-Wikipedia-Carbon.txt", "doc001");
        s_toDelete.Add(docId);
        Console.WriteLine($"- Document Id: {docId}");
    }
    ...
    {
        if (!await s_memory.IsDocumentReadyAsync("doc002"))
        {
    ...
    {
        if (!await s_memory.IsDocumentReadyAsync("doc003"))
        {
    ...
    private static async Task StoreWebPage()
    {
        if (!await s_memory.IsDocumentReadyAsync("webPage1"))
        {
            ...
            var docId = await s_memory.ImportWebPageAsync("https://raw.githubusercontent.com/microsoft/kernel-memory/main/README.md", "webPage1");
            s_toDelete.Add(docId);
            Console.WriteLine($"- Document Id: {docId}");
        }
        else
    ...
    {
        if (!await s_memory.IsDocumentReadyAsync("htmlDoc001"))
        {
    ...
    {
        if (!await s_memory.IsDocumentReadyAsync("xls01"))
        {
    ...
    {
        if (!await s_memory.IsDocumentReadyAsync("json01"))
        {
    ...
    */
    ...
    private static async Task AskSimpleQuestionStreamingAndShowSources()
    {
        ...
        Console.Write("\nAnswer: ");
        var answerStream = s_memory.AskStreamingAsync(question,
            minRelevance: 0.5,
    ...
    */
    ...
    private static async Task DownloadFile()
    {
        ...
        StreamableFileContent result = await s_memory.ExportFileAsync("doc001", Filename);

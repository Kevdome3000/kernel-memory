...
public class SimpleVectorDbTest : BaseUnitTestCase
{
    ...
    public async Task ItSearchesAllRecords()
    {
        ...
        {
            await target.UpsertAsync(index,
                new MemoryRecord
                {
        ...
        const int count = 25;
        List<(MemoryRecord, double)> result1 = await target.GetSimilarListAsync(index,
                text,
                minRelevance: 0.3,
                limit: count)
            .ToListAsync();
        ...
        List<(MemoryRecord, double)> result2 = await target.GetSimilarListAsync(index,
                text,
                minRelevance: 0.3,
                limit: -1)
            .ToListAsync();

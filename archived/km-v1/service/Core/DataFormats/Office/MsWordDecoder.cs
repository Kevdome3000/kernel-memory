...
public sealed class MsWordDecoder : IContentDecoder
{
    ...
    {
        _log = (loggerFactory ?? DefaultLogger.Factory).CreateLogger<MsWordDecoder>();
    }
    ...
    public Task<FileContent> DecodeAsync(Stream data, CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            if (paragraphs != null)
            {
                foreach (Paragraph p in paragraphs)
                {
                    ...
                    if (lastRenderedPageBreak != null)
                    {
                        ...
                        string pageContent = sb.ToString().NormalizeNewlines();
                        sb.Clear();
                        result.Sections.Add(new Chunk(pageContent, pageNumber, Chunk.Meta(true)));
                        pageNumber++;
                    }
            ...
            // Note: no trimming, use original spacing when working with pages
            string lastPageContent = sb.ToString().NormalizeNewlines();
            result.Sections.Add(new Chunk(lastPageContent, pageNumber, Chunk.Meta(true)));

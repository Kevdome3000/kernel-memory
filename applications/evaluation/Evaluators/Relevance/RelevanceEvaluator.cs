...
internal sealed class RelevanceEvaluator : EvaluationEngine
{
    ...
    private KernelFunction ExtractQuestion => _kernel.CreateFunctionFromPrompt(GetSKPrompt("Extraction", "Question"),
    ...
    {
        var questionEmbeddings = await _textEmbeddingGenerationService
            .GenerateEmbeddingsAsync([answer.Question], _kernel)
        ...
        var generatedQuestionsEmbeddings = await _textEmbeddingGenerationService
            .GenerateEmbeddingsAsync(generatedQuestions.Select(c => c.Question).ToArray(), _kernel)
            .ConfigureAwait(false);
    ...
    private async IAsyncEnumerable<RelevanceEvaluation> GetEvaluations(MemoryAnswer answer, int strictness)
    {
        foreach (var item in Enumerable.Range(0, strictness))
        {
            var statements = await Try(3,
                    async remainingTry =>
                    {
                        var extraction = await ExtractQuestion.InvokeAsync(_kernel,
                                new KernelArguments
                                {

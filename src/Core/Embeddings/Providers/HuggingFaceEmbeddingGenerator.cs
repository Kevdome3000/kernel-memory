...
public sealed class HuggingFaceEmbeddingGenerator : IEmbeddingGenerator
{
        ...
        Func<TimeSpan, CancellationToken, Task>? delayAsync = null)
    {
        ...
        _apiKey = apiKey;
        _baseUrl = (baseUrl ?? Constants.EmbeddingDefaults.DefaultHuggingFaceBaseUrl).TrimEnd('/');
        ...
        _logger.LogDebug("HuggingFaceEmbeddingGenerator initialized: {BaseUrl}, model: {Model}, dimensions: {Dimensions}",
            _baseUrl,
            ModelName,
            VectorDimensions);
    }
    ...
    {
        var endpoint = $"{_baseUrl}/models/{ModelName}";
        ...
        _logger.LogTrace("Calling HuggingFace embeddings API: {Endpoint}, batch size: {BatchSize}",
            endpoint,
            textArray.Length);
        ...
        _logger.LogTrace("HuggingFace returned {Count} embeddings with {Dimensions} dimensions each",
            embeddings.Length,
            embeddings[0].Length);

...
public struct Embedding : IEquatable<Embedding>
{
    ...
    {
        var size1 = Data.Span.Length;
        var size2 = embedding.Data.Span.Length;

        if (size1 != size2)
        {
            throw new InvalidOperationException(
                "Embedding vectors must have the same length to calculate cosine similarity. " + $"Embedding 1 length: {size1}; Embedding 2 length: {size2}.");
        }

        return TensorPrimitives.CosineSimilarity(Data.Span, embedding.Data.Span);
    }
    ...
    /// </summary>
    public static implicit operator Embedding(ReadOnlyMemory<float> data)
    {
        return new Embedding(data);
    }
    ...
    /// </summary>
    public static implicit operator Embedding(float[] data)
    {
        return new Embedding(data);
    }
    ...
    {
        return (obj is Embedding other && Equals(other));
    }
    ...
    public sealed class JsonConverter : JsonConverter<Embedding>
    {
        ...
        {
            s_converter.Write(writer,
                MemoryMarshal.TryGetArray(value.Data, out ArraySegment<float> array) && array.Count == value.Length
                    ? array.Array!
                    : value.Data.ToArray(),
                options);
        }

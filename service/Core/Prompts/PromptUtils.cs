...
internal static class PromptUtils
{
        ...
        Dictionary<string, object>? metadata = null)
    {
        ...
        {
            result = s_tagsRegex.Replace(result,
                match =>
                {
        ...
        {
            result = s_metadataRegex.Replace(result,
                match =>
                {
                    if (metadata != null)
                    {
                        string metadataKey = match.Groups[1].Value;
                        return metadata.TryGetValue(metadataKey, out object? metadataValue)
                            ? $"{metadataValue}"
                            : "-";
                    }

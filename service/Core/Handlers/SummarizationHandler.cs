...
public sealed class SummarizationHandler : IPipelineStepHandler
{
        ...
        ILoggerFactory? loggerFactory = null)
    {
        ...
        _log = (loggerFactory ?? DefaultLogger.Factory).CreateLogger<SummarizationHandler>();
            ...
            foreach (KeyValuePair<string, DataPipeline.GeneratedFileDetails> generatedFile in uploadedFile.GeneratedFiles)
            {
                ...
                switch (file.MimeType)
                {
                    ...
                    case MimeTypes.MarkDown:
                        ...
                        (string summary, bool success) = await SummarizeAsync(content, pipeline.GetContext()).ConfigureAwait(false);
    ...
    private async Task<(string summary, bool skip)> SummarizeAsync(string content, IContext context)
    {
        ...
        _log.LogTrace("Size of the content to summarize: {0} tokens", contentLength);

        // If the content is less than 30 tokens don't do anything and move on.
        if (contentLength < MinLength)
        {
            _log.LogWarning("Content is too short to summarize ({0} tokens), nothing to do", contentLength);
            return (content, true);
        }
        ...
        _log.LogTrace("Target goal: summary max size <= {0} tokens", targetSummarySize);
        ...
        _log.LogTrace("Overlap setting: {0} tokens", overlappingTokens);

        // Summarize at least once
        var done = false;

        var summarizationPrompt = context.GetCustomSummaryPromptOrDefault(_summarizationPrompt);
        ...
        while (!done)
        {
            ...
            {
                chunks = _plainTextChunker.Split(content, new PlainTextChunkerOptions { MaxTokensPerChunk = maxTokensPerParagraph, Overlap = overlappingTokens });
            }

            _log.LogTrace("Paragraphs to summarize: {0}", chunks.Count);

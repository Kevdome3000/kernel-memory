...
public sealed class TextPartitioningHandler : IPipelineStepHandler
{
        ...
        ILoggerFactory? loggerFactory = null)
    {
        ...
        _plainTextChunker = new PlainTextChunker(new CL100KTokenizer());
        _markDownChunker = new MarkDownChunker(new CL100KTokenizer());
        ...
        if (orchestrator.EmbeddingGenerationEnabled)
        {
            ...
            {
                throw ChunkTooBigForEmbeddingsException(_options.MaxTokensPerParagraph, _maxTokensPerPartition, _log);
            }
        }
    }
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var maxTokensPerChunk = context.GetCustomPartitioningMaxTokensPerChunkOrDefault(_options.MaxTokensPerParagraph);

        if (maxTokensPerChunk > _maxTokensPerPartition)
        {
            throw ChunkTooBigForEmbeddingsException(maxTokensPerChunk, _maxTokensPerPartition, _log);
        }
        ...
        foreach (DataPipeline.FileDetails uploadedFile in pipeline.Files)
        {
            ...
            foreach (KeyValuePair<string, DataPipeline.GeneratedFileDetails> generatedFile in uploadedFile.GeneratedFiles)
            {
                ...
                List<string> chunks;
                BinaryData fileContent = await _orchestrator.ReadFileAsync(pipeline, file.Name, cancellationToken).ConfigureAwait(false);
                ...
                switch (file.MimeType)
                {
                    case MimeTypes.PlainText:
                    {
                        ...
                        chunks = _plainTextChunker.Split(content, new PlainTextChunkerOptions { MaxTokensPerChunk = maxTokensPerChunk, Overlap = overlappingTokens, ChunkHeader = chunkHeader });
                        break;
                    }
                    ...
                    case MimeTypes.MarkDown:
                    {
                        ...
                        chunks = _markDownChunker.Split(content, new MarkDownChunkerOptions { MaxTokensPerChunk = maxTokensPerChunk, Overlap = overlappingTokens, ChunkHeader = chunkHeader });
                        break;
                    }
                ...
                _log.LogDebug("Saving {0} file partitions", chunks.Count);
...
#pragma warning disable CA2254 // the msg is always used
    ...
    {
        var errMsg = $"The configured partition size ({value} tokens) is too big for one " + $"of the embedding generators in use. The max value allowed is {limit} tokens. " + $"Consider changing the partitioning options, see {InternalConstants.DocsBaseUrl}/how-to/custom-partitioning for details.";

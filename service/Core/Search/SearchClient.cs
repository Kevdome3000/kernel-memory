...
public sealed class SearchClient : ISearchClient
{
        ...
        ILoggerFactory? loggerFactory = null)
    {
        ...
        _log = (loggerFactory ?? DefaultLogger.Factory).CreateLogger<SearchClient>();
        ...
        _answerGenerator = new AnswerGenerator(textGenerator,
            config,
            promptProvider,
            contentModeration,
            loggerFactory);
    ...
    {
        return _memoryDb.GetIndexesAsync(cancellationToken);
    }
...
#pragma warning disable CA2254
        _log.LogTrace(string.IsNullOrEmpty(query)
...
#pragma warning restore CA2254
        ...
        IAsyncEnumerable<(MemoryRecord, double)> matches = string.IsNullOrEmpty(query)
            ? _memoryDb.GetListAsync(index,
                    filters,
                    limit,
                    false,
                    cancellationToken)
                .Select(memoryRecord => (memoryRecord, double.MinValue))
            : _memoryDb.GetSimilarListAsync(index,
                query,
                filters,
                minRelevance,
                limit,
        ...
        await foreach ((MemoryRecord memoryRecord, double recordRelevance) in matches.ConfigureAwait(false).WithCancellation(cancellationToken))
        {
            ...
            result = ProcessMemoryRecord(result,
                index,
                memoryRecord,
                recordRelevance);
        ...
        _log.LogTrace("{Count} records processed", result.RecordCount);
        ...
        [EnumeratorCancellation] CancellationToken cancellationToken = default)
    {
        ...
        string answerPrompt = context.GetCustomRagPromptOrDefault(_answerPrompt);
        int limit = context.GetCustomRagMaxMatchesCountOrDefault(_config.MaxMatchesCount);
        bool includeDuplicateFacts = context.GetCustomRagIncludeDuplicateFactsOrDefault(_config.IncludeDuplicateFacts);

        var maxTokens = _config.MaxAskPromptSize > 0
            ? _config.MaxAskPromptSize
            : _textGenerator.MaxTokenTotal;

        // Prepare results (empty, error, etc.)
        SearchClientResult result = SearchClientResult.AskResultInstance(
            question,
            emptyAnswer,
            _config.ModeratedAnswer,
            limit,
            maxTokens
            - _textGenerator.CountTokens(answerPrompt)
            - _textGenerator.CountTokens(question)
            - _config.AnswerTokens
        );

        if (string.IsNullOrEmpty(question))
        {
            _log.LogWarning("No question provided");
            yield return result.NoQuestionResult;

            yield break;
        }

        _log.LogTrace("Fetching relevant memories");
        ...
        string factTemplate = context.GetCustomRagFactTemplateOrDefault(_config.FactTemplate);
        ...
        await foreach (MemoryAnswer answer in _answerGenerator.GenerateAnswerAsync(question,
                result,
                context,
                cancellationToken)
            .ConfigureAwait(false))
    ...
    private SearchClientResult ProcessMemoryRecord(
        ...
        string index,
        MemoryRecord record,
        double recordRelevance,
        bool includeDupes = true,
        string? factTemplate = null)
    {
        var partitionText = record.GetPartitionText(_log).Trim();

        if (string.IsNullOrEmpty(partitionText))
        {
            _log.LogError("The document partition is empty, doc: {0}", record.Id);
        ...
        string documentId = record.GetDocumentId(_log);

        // Identify the file in case there are multiple files
        string fileId = record.GetFileId(_log);
        ...
        string fileName = record.GetFileName(_log);
        ...
        string fileNameForLLM = fileName == "content.url"
            ? fileDownloadUrl
            : fileName;
        ...
        if (result.Mode == SearchMode.SearchMode)
        {
            ...
            if (recordRelevance > float.MinValue) { _log.LogTrace("Adding result with relevance {0}", recordRelevance); }
        }
        else if (result.Mode == SearchMode.AskMode)
        {
            ...
            if (!skipFactInPrompt)
            {
                string fact = PromptUtils.RenderFactTemplate(
                    ...
                    partitionText,
                    fileNameForLLM,
                    recordRelevance.ToString("P1", CultureInfo.CurrentCulture),
                    record.Id,
                    record.Tags,
                    record.Payload);

                // Use the partition/chunk only if there's room for it
                int factSizeInTokens = _textGenerator.CountTokens(fact);
                ...
                _log.LogTrace("Adding content #{FactsUsedCount} with relevance {Relevance} (dupe: {IsDupe})",
                    result.FactsUsedCount,
                    recordRelevance,
                    isDupe);
            }
        ...
        citation.SourceContentType = record.GetFileContentType(_log);
        ...
        citation.Partitions.Add(new Citation.Partition
        {
            ...
            PartitionNumber = record.GetPartitionNumber(_log),

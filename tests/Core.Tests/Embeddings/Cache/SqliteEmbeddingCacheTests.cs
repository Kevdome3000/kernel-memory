...
public sealed class SqliteEmbeddingCacheTests : IDisposable
{
    ...
    {
        _tempDbPath = Path.Combine(Path.GetTempPath(), $"test-cache-{Guid.NewGuid()}.db");
    ...
    {
        // Arrange
        using var cache = new SqliteEmbeddingCache(_tempDbPath, CacheModes.ReadWrite, _loggerMock.Object);
    ...
    public async Task StoreAsync_WithSameKey_ShouldOverwrite()
    {
        ...
        var key = EmbeddingCacheKey.Create("OpenAI",
            ...
            true,
            "test text");
        var vector1 = new[] { 0.1f, 0.2f, 0.3f };
        var vector2 = new[] { 0.9f, 0.8f, 0.7f };
    ...
    public async Task TryGetAsync_WithDifferentKeys_ShouldReturnCorrectValues()
    {
        ...
        var key2 = EmbeddingCacheKey.Create("OpenAI",
            ...
            "text two");
        var vector1 = new[] { 0.1f, 0.2f, 0.3f };
        var vector2 = new[] { 0.4f, 0.5f, 0.6f };

        // Act
        await cache.StoreAsync(key1,
                vector1,
                null,
                CancellationToken.None)
            .ConfigureAwait(false);
        await cache.StoreAsync(key2,
                vector2,
                null,
                CancellationToken.None)
            .ConfigureAwait(false);
    ...
    public async Task ReadOnlyMode_TryGetAsync_ShouldWork()
    {
        ...
        using (var writeCache = new SqliteEmbeddingCache(_tempDbPath, CacheModes.ReadWrite, _loggerMock.Object))
        {
            var key = EmbeddingCacheKey.Create("OpenAI",
                "model",
                1536,
            ...
            await writeCache.StoreAsync(key,
                    vector,
                    null,
                    CancellationToken.None)
                .ConfigureAwait(false);
        ...
        using var readCache = new SqliteEmbeddingCache(_tempDbPath, CacheModes.ReadOnly, _loggerMock.Object);
        var readKey = EmbeddingCacheKey.Create("OpenAI",
            "model",
            1536,
            true,
            "test text");
    ...
    {
        // Arrange
        using var cache = new SqliteEmbeddingCache(_tempDbPath, CacheModes.ReadOnly, _loggerMock.Object);
        var key = EmbeddingCacheKey.Create("OpenAI",
            "model",
            1536,
            true,
    ...
    {
        // Arrange
        using var cache = new SqliteEmbeddingCache(_tempDbPath, CacheModes.WriteOnly, _loggerMock.Object);
        var key = EmbeddingCacheKey.Create("OpenAI",
            "model",
            1536,
            true,
        ...
        using var readCache = new SqliteEmbeddingCache(_tempDbPath, CacheModes.ReadWrite, _loggerMock.Object);
    ...
    {
        // Arrange & Act
        using var readWriteCache = new SqliteEmbeddingCache(_tempDbPath, CacheModes.ReadWrite, _loggerMock.Object);
        var path2 = Path.Combine(Path.GetTempPath(), $"test-cache-{Guid.NewGuid()}.db");
        using var readOnlyCache = new SqliteEmbeddingCache(path2, CacheModes.ReadOnly, _loggerMock.Object);
        var path3 = Path.Combine(Path.GetTempPath(), $"test-cache-{Guid.NewGuid()}.db");
        using var writeOnlyCache = new SqliteEmbeddingCache(path3, CacheModes.WriteOnly, _loggerMock.Object);
    ...
    public async Task CacheDoesNotStoreInputText()
    {
        ...
        var dbContent = await File.ReadAllBytesAsync(_tempDbPath).ConfigureAwait(false);
        var dbString = Encoding.UTF8.GetString(dbContent);
    ...
    public async Task CachePersistence_ShouldSurviveReopen()
    {
        ...
        // Act - Reopen and read
        using var reopenedCache = new SqliteEmbeddingCache(_tempDbPath, CacheModes.ReadWrite, _loggerMock.Object);
    ...
    public async Task StoreAsync_WithCancellationToken_ShouldRespectCancellation()
    {
        ...
        // Act & Assert
        await Assert.ThrowsAsync<OperationCanceledException>(() => cache.StoreAsync(key,
                vector,
                null,
                cts.Token))
            .ConfigureAwait(false);
    ...
    public async Task TryGetAsync_WithCancellationToken_ShouldRespectCancellation()
    {
        ...
        // Act & Assert
        await Assert.ThrowsAsync<OperationCanceledException>(() => cache.TryGetAsync(key, cts.Token)).ConfigureAwait(false);
    }
}

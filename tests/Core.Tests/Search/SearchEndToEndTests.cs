...
public sealed class SearchEndToEndTests : IDisposable
{
    ...
    {
        _tempDir = Path.Combine(Path.GetTempPath(), $"km-e2e-search-{Guid.NewGuid():N}");
        ...
        _searchService = new SearchService(new Dictionary<string, NodeSearchService> { ["test-node"] = nodeService });

        _insertedIds = new Dictionary<string, string>();
    }
    ...
    private async Task InsertAsync(
        ...
        string[]? tags = null)
    {
        var result = await _storage.UpsertAsync(new UpsertRequest
                {
                    Content = content,
    ...
    {
        return await _searchService.SearchAsync(new SearchRequest
                {
                    Query = query,
                    Limit = limit,
    ...
    {
        // Arrange
        await InsertAsync("doc1", "kubernetes deployment guide").ConfigureAwait(false);
        await InsertAsync("doc2", "docker container basics").ConfigureAwait(false);
        await InsertAsync("doc3", "python programming tutorial").ConfigureAwait(false);

        // Act: Simple text search
        var response = await SearchAsync("kubernetes").ConfigureAwait(false);

        // Assert: Verify actual results
        Assert.Equal(1, response.TotalResults);
        Assert.Single(response.Results);
        Assert.Equal(_insertedIds["doc1"], response.Results[0].Id);
    ...
    {
        // Arrange
        await InsertAsync("doc1", "kubernetes and docker together").ConfigureAwait(false);
        await InsertAsync("doc2", "only kubernetes here").ConfigureAwait(false);
        await InsertAsync("doc3", "only docker here").ConfigureAwait(false);

        // Act: AND operator
        var response = await SearchAsync("kubernetes AND docker").ConfigureAwait(false);
    ...
    {
        // Arrange
        await InsertAsync("doc1", "python programming").ConfigureAwait(false);
        await InsertAsync("doc2", "javascript development").ConfigureAwait(false);
        await InsertAsync("doc3", "java enterprise").ConfigureAwait(false);

        // Act: OR operator
        var response = await SearchAsync("python OR javascript").ConfigureAwait(false);
        ...
        Assert.Contains(_insertedIds["doc1"], resultIds);
        Assert.Contains(_insertedIds["doc2"], resultIds);
        Assert.DoesNotContain(_insertedIds["doc3"], resultIds);
    }
    ...
    {
        // Arrange
        await InsertAsync("doc1", "docker and kubernetes tutorial").ConfigureAwait(false);
        await InsertAsync("doc2", "docker and helm charts").ConfigureAwait(false);
        await InsertAsync("doc3", "terraform and kubernetes").ConfigureAwait(false);
        await InsertAsync("doc4", "ansible automation").ConfigureAwait(false);

        // Act: Complex nested query
        var response = await SearchAsync("docker AND (kubernetes OR helm)").ConfigureAwait(false);
        ...
        Assert.Contains(_insertedIds["doc1"], resultIds);
        Assert.Contains(_insertedIds["doc2"], resultIds);
        Assert.DoesNotContain(_insertedIds["doc3"], resultIds); // Has kubernetes but no docker
        Assert.DoesNotContain(_insertedIds["doc4"], resultIds); // Has neither
    }
    ...
    {
        // Arrange
        await InsertAsync("doc1", "database configuration", "Setup Guide").ConfigureAwait(false);
        await InsertAsync("doc2", "user authentication", "Database Configuration").ConfigureAwait(false);

        // Act: Search specifically in content field
        var response = await SearchAsync("content:database").ConfigureAwait(false);
    ...
    {
        // Arrange
        await InsertAsync("doc1", "how to configure docker", "Docker Tutorial").ConfigureAwait(false);
        await InsertAsync("doc2", "kubernetes deployment with docker", "Kubernetes Guide").ConfigureAwait(false);

        // Act: Search specifically in title field
        var response = await SearchAsync("title:docker").ConfigureAwait(false);
    ...
    {
        // Arrange
        await InsertAsync("doc1",
                "api documentation",
                "REST API Guide",
                "Complete guide to REST APIs")
            .ConfigureAwait(false);
        await InsertAsync("doc2",
                "graphql tutorial",
                "GraphQL API",
                "Learn GraphQL APIs")
            .ConfigureAwait(false);
        await InsertAsync("doc3",
                "database setup",
                "Database Guide",
                "Setup your database")
            .ConfigureAwait(false);
        ...
        var response = await SearchAsync("(title:api OR description:api) AND content:documentation").ConfigureAwait(false);
    ...
    {
        // Arrange
        await InsertAsync("doc1", "summary of the meeting").ConfigureAwait(false);
        await InsertAsync("doc2", "detailed report").ConfigureAwait(false);

        // Act: Search for "summaries" (plural) should find "summary" (singular)
        var response = await SearchAsync("content:summaries").ConfigureAwait(false);
    ...
    {
        // Arrange
        await InsertAsync("doc1", "guide content", "Connect to server").ConfigureAwait(false);
        await InsertAsync("doc2", "other content", "Setup instructions").ConfigureAwait(false);

        // Act: Search for "connection" should find "connect"
        var response = await SearchAsync("title:connection").ConfigureAwait(false);
    ...
    {
        // Arrange
        await InsertAsync("doc1", "Kubernetes Tutorial").ConfigureAwait(false);
        await InsertAsync("doc2", "DOCKER GUIDE").ConfigureAwait(false);

        // Act: Search with different casing
        var response1 = await SearchAsync("KUBERNETES").ConfigureAwait(false);
        var response2 = await SearchAsync("kubernetes").ConfigureAwait(false);
        var response3 = await SearchAsync("KuBeRnEtEs").ConfigureAwait(false);
        ...
        Assert.Equal(_insertedIds["doc1"], response1.Results[0].Id);
        Assert.Equal(_insertedIds["doc1"], response2.Results[0].Id);
        Assert.Equal(_insertedIds["doc1"], response3.Results[0].Id);
    }
    ...
    {
        // Arrange
        await InsertAsync("doc1", "machine learning algorithms").ConfigureAwait(false);
        await InsertAsync("doc2", "deep learning networks").ConfigureAwait(false);

        // Act: Search with default MinRelevance (0.3)
        var response = await SearchAsync("learning", 0.3f).ConfigureAwait(false);
    ...
    public async Task InfixQuery_ThreeFieldQuery_FindsAcrossAllFields()
    {
        ...
        var response = await SearchAsync("title:api OR description:api OR content:api").ConfigureAwait(false);
    ...
    {
        // Arrange
        await InsertAsync("doc1", "kubernetes orchestration").ConfigureAwait(false);
        await InsertAsync("doc2", "docker containers").ConfigureAwait(false);

        // Act: MongoDB JSON syntax
        var response = await SearchAsync("{\"content\": \"kubernetes\"}").ConfigureAwait(false);
    ...
    {
        // Arrange
        await InsertAsync("doc1", "docker and kubernetes").ConfigureAwait(false);
        await InsertAsync("doc2", "only docker").ConfigureAwait(false);
        await InsertAsync("doc3", "only kubernetes").ConfigureAwait(false);

        // Act: MongoDB $and
        var response = await SearchAsync("{\"$and\": [{\"content\": \"docker\"}, {\"content\": \"kubernetes\"}]}").ConfigureAwait(false);
    ...
    {
        // Arrange
        await InsertAsync("doc1", "python code").ConfigureAwait(false);
        await InsertAsync("doc2", "javascript code").ConfigureAwait(false);
        await InsertAsync("doc3", "java code").ConfigureAwait(false);

        // Act: MongoDB $or
        var response = await SearchAsync("{\"$or\": [{\"content\": \"python\"}, {\"content\": \"javascript\"}]}").ConfigureAwait(false);
    ...
    {
        // Arrange
        await InsertAsync("doc1", "full text search capabilities").ConfigureAwait(false);
        await InsertAsync("doc2", "vector search features").ConfigureAwait(false);

        // Act: MongoDB $text operator
        var response = await SearchAsync("{\"$text\": {\"$search\": \"full text\"}}").ConfigureAwait(false);

        // Assert
        Assert.True(response.TotalResults > 0);
        Assert.Contains(response.Results, r => r.Id == _insertedIds["doc1"]);
    }
    ...
    {
        // Arrange
        await InsertAsync("doc1", "development guide", "Develop Features").ConfigureAwait(false);
        await InsertAsync("doc2", "deployment process", "Deploy Apps").ConfigureAwait(false);

        // Act: Search for "development" in title should find "develop"
        var response = await SearchAsync("{\"title\": \"development\"}").ConfigureAwait(false);
    ...
    {
        // Arrange
        await InsertAsync("doc1", "api documentation for rest services", "REST API").ConfigureAwait(false);
        await InsertAsync("doc2", "graphql api tutorial", "GraphQL Guide").ConfigureAwait(false);
        await InsertAsync("doc3", "database rest interface", "Database API").ConfigureAwait(false);
        ...
        var resultIds = response.Results.Select(r => r.Id).ToHashSet();
        Assert.Contains(_insertedIds["doc1"], resultIds);
        Assert.Contains(_insertedIds["doc3"], resultIds);
        Assert.DoesNotContain(_insertedIds["doc2"], resultIds); // Title lacks "api"
    }
    ...
    {
        // Arrange
        await InsertAsync("doc1", "kubernetes deployment").ConfigureAwait(false);
        await InsertAsync("doc2", "docker deployment").ConfigureAwait(false);
        await InsertAsync("doc3", "helm charts").ConfigureAwait(false);

        // Act: Same query in both formats
        var infixResponse = await SearchAsync("kubernetes OR docker").ConfigureAwait(false);
        var mongoResponse = await SearchAsync("{\"$or\": [{\"content\": \"kubernetes\"}, {\"content\": \"docker\"}]}").ConfigureAwait(false);
    ...
    {
        // Arrange
        await InsertAsync("doc1", "docker and kubernetes together").ConfigureAwait(false);
        await InsertAsync("doc2", "only docker here").ConfigureAwait(false);
        await InsertAsync("doc3", "only kubernetes here").ConfigureAwait(false);

        // Act: Complex AND query in both formats
        var infixResponse = await SearchAsync("docker AND kubernetes").ConfigureAwait(false);
        var mongoResponse = await SearchAsync("{\"$and\": [{\"content\": \"docker\"}, {\"content\": \"kubernetes\"}]}").ConfigureAwait(false);
        ...
        Assert.Equal(_insertedIds["doc1"], infixResponse.Results[0].Id);
    }
    ...
    public async Task Search_WithPagination_ReturnsCorrectSubset()
    {
        ...
        for (int i = 0; i < 10; i++)
        {
            await InsertAsync($"doc{i}", $"test document number {i}").ConfigureAwait(false);
        }

        // Act: Get results with pagination
        var page1 = await _searchService.SearchAsync(new SearchRequest
                {
                    Query = "test",
                    Limit = 3,
                    Offset = 0,
        ...
        var page2 = await _searchService.SearchAsync(new SearchRequest
                {
                    Query = "test",
                    Limit = 3,
                    Offset = 3,
    ...
    public async Task Search_TotalResults_ReflectsFilteredCountBeforePagination()
    {
        ...
        for (int i = 0; i < 10; i++)
        {
            await InsertAsync($"doc{i}", $"test item {i}").ConfigureAwait(false);
        }
    ...
    public async Task RegressionTest_Bm25NormalizationBug_ScoresAboveMinRelevance()
    {
        ...
        // Arrange
        await InsertAsync("doc1", "simple test content").ConfigureAwait(false);

        // Act: Use default MinRelevance=0.3 (the value that exposed the bug)
        var response = await SearchAsync("test", 0.3f).ConfigureAwait(false);
    ...
    public async Task RegressionTest_FieldSpecificEqualOperator_ExtractsFtsQuery()
    {
        ...
        // Arrange
        await InsertAsync("doc1", "summary of findings").ConfigureAwait(false);
    ...
    public async Task KnownIssue2_QuotedPhraseWithAND_FindsExactPhrase()
    {
        ...
        // Arrange
        await InsertAsync("doc1", "Meeting with Alice AND Bob").ConfigureAwait(false);
        await InsertAsync("doc2", "Alice went to lunch and Bob stayed").ConfigureAwait(false);
        await InsertAsync("doc3", "Just Alice here").ConfigureAwait(false);
        await InsertAsync("doc4", "Just Bob here").ConfigureAwait(false);

        // Act: Search for the exact phrase "Alice AND Bob" using quotes
        var response = await SearchAsync("\"Alice AND Bob\"").ConfigureAwait(false);
    ...
    public async Task KnownIssue2_QuotedPhraseWithOR_FindsExactPhrase()
    {
        ...
        // Arrange
        await InsertAsync("doc1", "choose this OR that option").ConfigureAwait(false);
        await InsertAsync("doc2", "this is one option or that is another").ConfigureAwait(false);
        await InsertAsync("doc3", "just this").ConfigureAwait(false);
        await InsertAsync("doc4", "just that").ConfigureAwait(false);

        // Act: Search for the exact phrase "this OR that"
        var response = await SearchAsync("\"this OR that\"").ConfigureAwait(false);
    ...
    public async Task KnownIssue2_QuotedPhraseWithNOT_FindsExactPhrase()
    {
        ...
        // Arrange
        await InsertAsync("doc1", "this is NOT important notice").ConfigureAwait(false);
        await InsertAsync("doc2", "this is definitely important").ConfigureAwait(false);
        await InsertAsync("doc3", "NOT a problem").ConfigureAwait(false);

        // Act: Search for the exact phrase "this is NOT important"
        var response = await SearchAsync("\"this is NOT important\"").ConfigureAwait(false);
    ...
    public async Task KnownIssue2_QuotedReservedWordAND_FindsDocumentsContainingAND()
    {
        ...
        // Arrange
        await InsertAsync("doc1", "The word AND appears here").ConfigureAwait(false);
        await InsertAsync("doc2", "No reserved words").ConfigureAwait(false);

        // Act: Search for the literal word "AND"
        var response = await SearchAsync("\"AND\"").ConfigureAwait(false);
    ...
    public async Task KnownIssue2_QuotedReservedWordOR_FindsDocumentsContainingOR()
    {
        ...
        // Arrange
        await InsertAsync("doc1", "The word OR appears here").ConfigureAwait(false);
        await InsertAsync("doc2", "No reserved words").ConfigureAwait(false);

        // Act: Search for the literal word "OR"
        var response = await SearchAsync("\"OR\"").ConfigureAwait(false);
    ...
    public async Task KnownIssue2_QuotedReservedWordNOT_FindsDocumentsContainingNOT()
    {
        ...
        // Arrange
        await InsertAsync("doc1", "The word NOT appears here").ConfigureAwait(false);
        await InsertAsync("doc2", "No reserved words").ConfigureAwait(false);

        // Act: Search for the literal word "NOT"
        var response = await SearchAsync("\"NOT\"").ConfigureAwait(false);
    ...
    public async Task KnownIssue2_MixedQuotedPhraseAndOperator_WorksCorrectly()
    {
        ...
        // Arrange
        await InsertAsync("doc1", "Meeting notes for Alice AND Bob about kubernetes").ConfigureAwait(false);
        await InsertAsync("doc2", "Meeting notes for Alice AND Bob about docker").ConfigureAwait(false);
        await InsertAsync("doc3", "Meeting notes for Alice about kubernetes").ConfigureAwait(false);

        // Act: Search for exact phrase AND another term
        var response = await SearchAsync("\"Alice AND Bob\" AND kubernetes").ConfigureAwait(false);
    ...
    public async Task KnownIssue1_StandaloneNOT_DoesNotCrash()
    {
        ...
        // Arrange
        await InsertAsync("doc1", "contains foo term").ConfigureAwait(false);
        await InsertAsync("doc2", "does not contain the term").ConfigureAwait(false);
        await InsertAsync("doc3", "another document without it").ConfigureAwait(false);

        // Act: This should NOT throw an exception
        var response = await SearchAsync("NOT foo").ConfigureAwait(false);

        // Assert: Should return documents that do NOT contain "foo"
        Assert.Equal(2, response.TotalResults);
        var resultIds = response.Results.Select(r => r.Id).ToHashSet();
        Assert.DoesNotContain(_insertedIds["doc1"], resultIds); // Contains "foo"
        Assert.Contains(_insertedIds["doc2"], resultIds); // No "foo"
        Assert.Contains(_insertedIds["doc3"], resultIds); // No "foo"
    }
    ...
    public async Task KnownIssue1_NotWithPositiveTerm_ExcludesCorrectly()
    {
        ...
        // Arrange
        await InsertAsync("doc1", "foo and bar together").ConfigureAwait(false);
        await InsertAsync("doc2", "only foo here").ConfigureAwait(false);
        await InsertAsync("doc3", "only bar here").ConfigureAwait(false);
        await InsertAsync("doc4", "neither term here").ConfigureAwait(false);

        // Act: Search for "foo AND NOT bar" (explicit AND required)
        var response = await SearchAsync("foo AND NOT bar").ConfigureAwait(false);

        // Assert: Should return only doc2 (has "foo" but not "bar")
        Assert.Equal(1, response.TotalResults);
        Assert.Single(response.Results);
        Assert.Equal(_insertedIds["doc2"], response.Results[0].Id);
    }
    ...
    public async Task KnownIssue1_MultipleNOT_ExcludesAllTerms()
    {
        ...
        // Arrange
        await InsertAsync("doc1", "foo bar baz all").ConfigureAwait(false);
        await InsertAsync("doc2", "foo bar only").ConfigureAwait(false);
        await InsertAsync("doc3", "foo baz only").ConfigureAwait(false);
        await InsertAsync("doc4", "foo alone here").ConfigureAwait(false);

        // Act: Search for foo but not bar and not baz (explicit AND required)
        var response = await SearchAsync("foo AND NOT bar AND NOT baz").ConfigureAwait(false);

        // Assert: Should return only doc4 (has "foo" without "bar" or "baz")
        Assert.Equal(1, response.TotalResults);
        Assert.Single(response.Results);
        Assert.Equal(_insertedIds["doc4"], response.Results[0].Id);
    }
    ...
    public async Task KnownIssue1_NOTWithOR_WorksCorrectly()
    {
        ...
        // Arrange
        await InsertAsync("doc1", "foo and bar").ConfigureAwait(false);
        await InsertAsync("doc2", "foo alone").ConfigureAwait(false);
        await InsertAsync("doc3", "baz and bar").ConfigureAwait(false);
        await InsertAsync("doc4", "baz alone").ConfigureAwait(false);
        await InsertAsync("doc5", "neither term").ConfigureAwait(false);

        // Act: Search for (foo OR baz) AND NOT bar (explicit AND required)
        var response = await SearchAsync("(foo OR baz) AND NOT bar").ConfigureAwait(false);

        // Assert: Should return doc2 and doc4 (have foo/baz but not bar)
        Assert.Equal(2, response.TotalResults);
        var resultIds = response.Results.Select(r => r.Id).ToHashSet();
        Assert.Contains(_insertedIds["doc2"], resultIds);
        Assert.Contains(_insertedIds["doc4"], resultIds);
        Assert.DoesNotContain(_insertedIds["doc1"], resultIds); // Has bar
        Assert.DoesNotContain(_insertedIds["doc3"], resultIds); // Has bar
    }
    ...
    public async Task KnownIssue1_NOTInFieldQuery_ExcludesFromField()
    {
        ...
        // Arrange
        await InsertAsync("doc1", "foo bar in content", "title1").ConfigureAwait(false);
        await InsertAsync("doc2", "foo only in content", "title2").ConfigureAwait(false);
        await InsertAsync("doc3", "different content", "foo bar title").ConfigureAwait(false);

        // Act: Search for foo in content but not bar in content (explicit AND required)
        var response = await SearchAsync("content:foo AND NOT content:bar").ConfigureAwait(false);
    ...
    public async Task KnownIssue1_MongoNot_ExcludesCorrectly()
    {
        ...
        await InsertAsync("doc1", "kubernetes deployment").ConfigureAwait(false);
        await InsertAsync("doc2", "docker deployment").ConfigureAwait(false);
        await InsertAsync("doc3", "other content").ConfigureAwait(false);

        // Act: MongoDB NOT - find documents NOT containing "kubernetes"
        var response = await SearchAsync("{\"$not\": {\"content\": \"kubernetes\"}}").ConfigureAwait(false);

        // Assert: Should return doc2 and doc3 (not containing kubernetes)
        Assert.Equal(2, response.TotalResults);
        var resultIds = response.Results.Select(r => r.Id).ToHashSet();
        Assert.DoesNotContain(_insertedIds["doc1"], resultIds); // Contains kubernetes
        Assert.Contains(_insertedIds["doc2"], resultIds);
        Assert.Contains(_insertedIds["doc3"], resultIds);
    }
    ...
    public async Task KnownIssue1_MongoNor_ExcludesAllConditions()
    {
        ...
        var response = await SearchAsync("{\"$nor\": [{\"content\": \"kubernetes\"}, {\"content\": \"docker\"}]}").ConfigureAwait(false);

        // Assert: Should return doc3 and doc4 (not containing kubernetes or docker)
        Assert.Equal(2, response.TotalResults);
        var resultIds = response.Results.Select(r => r.Id).ToHashSet();
        Assert.DoesNotContain(_insertedIds["doc1"], resultIds); // Contains kubernetes
        Assert.DoesNotContain(_insertedIds["doc2"], resultIds); // Contains docker
        Assert.Contains(_insertedIds["doc3"], resultIds);
        Assert.Contains(_insertedIds["doc4"], resultIds);
    }
    ...
    public async Task KnownIssue1_NOTWithSingleQuotedReservedWord_ExcludesCorrectly()
    {
        ...
        // Arrange
        await InsertAsync("doc1", "Meeting with Alice AND Bob tomorrow").ConfigureAwait(false);
        await InsertAsync("doc2", "Regular meeting notes").ConfigureAwait(false);
        await InsertAsync("doc3", "Project status update").ConfigureAwait(false);

        // Act: NOT with single-quoted AND (reserved word) - should exclude docs containing literal AND
        var response = await SearchAsync("NOT 'AND'").ConfigureAwait(false);

        // Assert: Should return doc2 and doc3 (not containing "AND")
        Assert.Equal(2, response.TotalResults);
        var resultIds = response.Results.Select(r => r.Id).ToHashSet();
        Assert.DoesNotContain(_insertedIds["doc1"], resultIds); // Contains "AND"
        Assert.Contains(_insertedIds["doc2"], resultIds);
        Assert.Contains(_insertedIds["doc3"], resultIds);
    }
    ...
    public async Task KnownIssue1_NOTWithDoubleQuotedReservedWord_ExcludesCorrectly()
    {
        ...
        // Arrange
        await InsertAsync("doc1", "This is NOT important").ConfigureAwait(false);
        await InsertAsync("doc2", "Regular document content").ConfigureAwait(false);
        await InsertAsync("doc3", "Something else entirely").ConfigureAwait(false);

        // Act: NOT with double-quoted NOT (reserved word) - should exclude docs containing literal NOT
        var response = await SearchAsync("NOT \"NOT\"").ConfigureAwait(false);

        // Assert: Should return doc2 and doc3 (not containing "NOT")
        Assert.Equal(2, response.TotalResults);
        var resultIds = response.Results.Select(r => r.Id).ToHashSet();
        Assert.DoesNotContain(_insertedIds["doc1"], resultIds); // Contains "NOT"
        Assert.Contains(_insertedIds["doc2"], resultIds);
        Assert.Contains(_insertedIds["doc3"], resultIds);
    }

...
public sealed class ContentStorageServiceTests : IDisposable
{
    ...
    public async Task UpsertAsync_WithEmptyId_GeneratesNewIdAsync()
    {
        ...
        var content = await _service.GetByIdAsync(result.Id).ConfigureAwait(false);
    ...
    public async Task UpsertAsync_CalculatesByteSizeAsync()
    {
        ...
        var content = await _service.GetByIdAsync(result.Id).ConfigureAwait(false);
        Assert.NotNull(content);
        Assert.Equal(Encoding.UTF8.GetByteCount(testContent), content.ByteSize);
    }
    ...
    {
        // Arrange
        var customDate = new DateTimeOffset(2024,
            1,
            1,
            12,
            0,
    ...
    {
        // Act
        var result = await _service.ListAsync(0, 10).ConfigureAwait(false);
    ...
    public async Task ListAsync_ReturnsContentOrderedByCreationTimeDescending()
    {
        ...
        var result1 = await _service.UpsertAsync(new UpsertRequest { Content = "First", MimeType = "text/plain" }).ConfigureAwait(false);
        await Task.Delay(100).ConfigureAwait(false);
        var result2 = await _service.UpsertAsync(new UpsertRequest { Content = "Second", MimeType = "text/plain" }).ConfigureAwait(false);
        await Task.Delay(100).ConfigureAwait(false);
        var result3 = await _service.UpsertAsync(new UpsertRequest { Content = "Third", MimeType = "text/plain" }).ConfigureAwait(false);
    ...
    public async Task ListAsync_SupportsSkipAndTake()
    {
        ...
        for (int i = 0; i < 5; i++)
        {
            await _service.UpsertAsync(new UpsertRequest { Content = $"Content {i}", MimeType = "text/plain" }).ConfigureAwait(false);
            await Task.Delay(50).ConfigureAwait(false);
        }
        await Task.Delay(1000).ConfigureAwait(false); // Wait for processing

        // Act - Skip first 2, take next 2
        var result = await _service.ListAsync(2, 2).ConfigureAwait(false);

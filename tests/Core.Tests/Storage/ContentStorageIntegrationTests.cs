...
public sealed class ContentStorageIntegrationTests : IDisposable
{
    ...
    public async Task FullWorkflow_UpsertRetrieveDeleteAsync()
    {
        ...
        var updatedContent = await _service.GetByIdAsync(result.Id).ConfigureAwait(false);
        ...
        // Act 3: Delete
        await _service.DeleteAsync(result.Id).ConfigureAwait(false);
        await Task.Delay(200).ConfigureAwait(false); // Wait for processing

        // Assert 3: Content is deleted
        var deletedContent = await _service.GetByIdAsync(result.Id).ConfigureAwait(false);
    ...
    public async Task RealCuidGenerator_GeneratesValidIdsAsync()
    {
        ...
        for (int i = 0; i < 10; i++)
        {
            var result = await _service.UpsertAsync(new UpsertRequest
                {
                    Content = $"Content {i}",
                    MimeType = "text/plain"
                })
    ...
    public async Task DateTimeOffset_IsStoredAndRetrievedCorrectlyAsync()
    {
        ...
        var specificDate = new DateTimeOffset(2024,
            6,
            15,
            14,
            30,

...
using Microsoft.KernelMemory.MemoryDb.Redis.Internals;
...
public sealed class RedisMemory : IMemoryDb
{
        ...
        ILogger<RedisMemory>? logger = null)
    {
        ...
        _logger = logger ?? DefaultLogger<RedisMemory>.Instance;
        _search = multiplexer.GetDatabase().FT();
        _db = multiplexer.GetDatabase();
        _fieldNamesNoEmbeddings = config.Tags.Keys.Append(PayloadFieldName).Append(DistanceFieldName).ToArray();
    }
    ...
    {
        var normalizedIndexName = NormalizeIndexName(index, _config.AppPrefix);
        var schema = new Schema().AddVectorField(EmbeddingFieldName,
            _config.VectorAlgorithm,
    ...
    public async Task<IEnumerable<string>> GetIndexesAsync(CancellationToken cancellationToken = default)
    {
        ...
        return result.Select(x => (string)x!).Where(x => x.StartsWith($"{_config.AppPrefix}", StringComparison.Ordinal)).Select(x => x.Substring(_config.AppPrefix.Length));
    }
    ...
    public async Task DeleteIndexAsync(string index, CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            await _search.DropIndexAsync(normalizedIndexName, true).ConfigureAwait(false);
        }
    ...
    public async Task<string> UpsertAsync(string index, MemoryRecord record, CancellationToken cancellationToken = default)
    {
        ...
        {
            NormalizeIndexName(index, _config.AppPrefix),
            EmbeddingFieldName,
        ...
        {
            var isIndexed = _config.Tags.TryGetValue(item.Key, out var c);
            var separator = c ?? DefaultSeparator;

            if (!isIndexed)
            {
                _logger.LogError("Attempt to insert un-indexed tag field: '{Key}', will not be able to filter on it, please adjust the tag settings in your Redis Configuration", item.Key);
            ...
            {
                _logger.LogError("Attempted to insert record with tag field: {Key} containing the separator: '{Separator}'. " + "Update your {RedisConfig} to use a different separator, or remove the separator from the field.",
                    item.Key,
                    separator,
                    nameof(RedisConfig));
                throw new ArgumentException($"Attempted to insert record with tag field: '{item.Key}' containing the separator: '{separator}'. " + $"Update your {nameof(RedisConfig)} to use a different separator, or remove the separator from the field.");
            }
        ...
        var scriptResult = (await _db.ScriptEvaluateAsync(Scripts.CheckIndexAndUpsert, [key], args.ToArray()).ConfigureAwait(false)).ToString()!;

        if (scriptResult == "false")
        {
            await CreateIndexAsync(index, record.Vector.Length, cancellationToken).ConfigureAwait(false);
            await _db.ScriptEvaluateAsync(Scripts.CheckIndexAndUpsert, [key], args.ToArray()).ConfigureAwait(false);
        }
        ...
        if (filters != null && filters.Any(x => x.Pairs.Any()))
        {
            ...
            foreach (var filter in filters)
            {
                ...
                foreach ((string key, string? value) in filter.Pairs)
                {
                    if (value is null)
                    {
                        _logger.LogError("Attempted to perform null check on tag field. This behavior is not supported by Redis");
        ...
        [EnumeratorCancellation] CancellationToken cancellationToken = default)
    {
        ...
        if (filters != null && filters.Any(x => x.Pairs.Any()))
        {
            foreach ((string key, string? value) in filters.SelectMany(x => x.Pairs))
            {
                if (value is null)
                {
                    _logger.LogWarning("Attempted to perform null check on tag field. This behavior is not supported by Redis");
                }
    ...
    private (MemoryRecord, double) FromDocument(NRedisStack.Search.Document doc, bool withEmbedding)
    {
        ...
        foreach (var field in doc.GetProperties())
        {
            ...
            {
                _config.Tags.TryGetValue(field.Key, out var c);
                var separator = c ?? DefaultSeparator;
    ...
    private static string NormalizeIndexName(string index, string? prefix = null)
    {
        ...
        var indexWithPrefix = !string.IsNullOrWhiteSpace(prefix)
            ? $"{prefix}{index}"
            : index;

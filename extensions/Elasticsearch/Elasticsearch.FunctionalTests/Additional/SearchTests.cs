...
public class SearchTests : MemoryDbFunctionalTest
{
    ...
    public async Task CanGetListWithTagsAsync()
    {
        ...
        var docIds = await DataStorageTests.UpsertTextFilesAsync(
                ...
                Output,
                nameof(CanGetListWithTagsAsync),
                [
        ...
        Assert.Equal(ExpectedTotalParagraphs, docIds.Count());
        docIds = docIds.Select(x => x.Split("//")[0].Split("=")[1]).Distinct().ToList();

        Output.WriteLine($"Indexed returned the following ids:\n{string.Join("\n", docIds)}");
        ...
        var idx = 0;
        Output.WriteLine($"Filter: {filter.ToDebugString()}.\n");

        await foreach (var result in MemoryDb.GetListAsync(
            nameof(CanGetListWithTagsAsync),
            [filter],
            100))
        {
            var fileName = result.Payload["file"];
            Output.WriteLine($"Match #{idx++}: {fileName}");
        }

        Assert.Equal(ExpectedTotalParagraphs, idx);
    }
    ...
    public async Task CanGetListWithEmptyFiltersAsync()
    {
        await foreach (var result in MemoryDb.GetListAsync(
            nameof(CanGetListWithTagsAsync),
            [[]], // <-- KM has a test to make sure this works.
            100))
        {
        }

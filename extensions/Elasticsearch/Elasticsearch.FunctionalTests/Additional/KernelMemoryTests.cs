...
public class KernelMemoryTests : MemoryDbFunctionalTest
{
        ...
        : base(cfg, output)
    {
        KernelMemory = new KernelMemoryBuilder()
            ...
            .WithElasticsearchMemoryDb(ElasticsearchConfig)
    ...
    public async Task ItSupportsMultipleFiltersAsync()
    {
        ...
        while (!await KernelMemory.IsDocumentReadyAsync(Id, indexName))
        {
            Output.WriteLine("Waiting for memory ingestion to complete...");
            await Task.Delay(TimeSpan.FromSeconds(2));
        }

        // Multiple filters: unknown users cannot see the memory
        var answer = await KernelMemory.AskAsync("What is Orion?",
            filters:
            [
        ...
        answer = await KernelMemory.AskAsync("What is Orion?",
            filters:
            [
        ...
        await KernelMemory.DeleteDocumentAsync(Id, indexName);

        Output.WriteLine("Deleting index");
    ...
    public async Task ItSupportsTagsAsync()
    {
        ...
        while (!await KernelMemory.IsDocumentReadyAsync(Id).ConfigureAwait(false))
        {
            Output.WriteLine("Waiting for memory ingestion to complete...");
        ...
        var answer1 = await KernelMemory.AskAsync("What is Orion?", filter: MemoryFilters.ByTag("type", "news")).ConfigureAwait(false);
        Output.WriteLine("answer1: " + answer1.Result);

        while (retries-- > 0 && !answer1.Result.Contains("spacecraft", StringComparison.OrdinalIgnoreCase))
        {
            await Task.Delay(TimeSpan.FromSeconds(1)).ConfigureAwait(false);
            answer1 = await KernelMemory.AskAsync("What is Orion?", filter: MemoryFilters.ByTag("type", "news")).ConfigureAwait(false);
            Output.WriteLine("answer1: " + answer1.Result);
        }

        retries = defaultRetries;
        var answer2 = await KernelMemory.AskAsync("What is Orion?", filter: MemoryFilters.ByTag("type", "test")).ConfigureAwait(false);
        Output.WriteLine("answer2: " + answer2.Result);

        while (retries-- > 0 && !answer2.Result.Contains("spacecraft", StringComparison.OrdinalIgnoreCase))
        {
            await Task.Delay(TimeSpan.FromSeconds(1)).ConfigureAwait(false);
            answer2 = await KernelMemory.AskAsync("What is Orion?", filter: MemoryFilters.ByTag("type", "test")).ConfigureAwait(false);
            Output.WriteLine("answer2: " + answer2.Result);
        }

        retries = defaultRetries;
        var answer3 = await KernelMemory.AskAsync("What is Orion?", filter: MemoryFilters.ByTag("ext", "pdf")).ConfigureAwait(false);
        Output.WriteLine("answer3: " + answer3.Result);

        while (retries-- > 0 && !answer3.Result.Contains("spacecraft", StringComparison.OrdinalIgnoreCase))
        {
            await Task.Delay(TimeSpan.FromSeconds(1)).ConfigureAwait(false);
            answer3 = await KernelMemory.AskAsync("What is Orion?", filter: MemoryFilters.ByTag("type", "test")).ConfigureAwait(false);
            Output.WriteLine("answer3: " + answer3.Result);
        }

        var answer4 = await KernelMemory.AskAsync("What is Orion?", filter: MemoryFilters.ByTag("foo", "bar")).ConfigureAwait(false);
        Output.WriteLine(answer4.Result);
    ...
    public async Task ItSupportsASingleFilterAsync()
    {
        ...
        while (!await KernelMemory.IsDocumentReadyAsync(Id, indexName).ConfigureAwait(false))
        {
            Output.WriteLine("Waiting for memory ingestion to complete...");
            await Task.Delay(TimeSpan.FromSeconds(2)).ConfigureAwait(false);
        }
        ...
        answer = await KernelMemory.AskAsync("What is Orion?", filter: MemoryFilters.ByTag("user", "someone"), index: indexName).ConfigureAwait(false);
        Output.WriteLine(answer.Result);
        Assert.Contains(NotFound, answer.Result, StringComparison.OrdinalIgnoreCase);

        // Simple filter: test AND logic: valid type + invalid user
        answer = await KernelMemory.AskAsync("What is Orion?", filter: MemoryFilters.ByTag("type", "news").ByTag("user", "someone"), index: indexName).ConfigureAwait(false);
        Output.WriteLine(answer.Result);
        Assert.Contains(NotFound, answer.Result, StringComparison.OrdinalIgnoreCase);

        // Simple filter: test AND logic: invalid type + valid user
        answer = await KernelMemory.AskAsync("What is Orion?", filter: MemoryFilters.ByTag("type", "fact").ByTag("user", "owner"), index: indexName).ConfigureAwait(false);
        Output.WriteLine(answer.Result);
        //Assert.Contains(Found, answer.Result, StringComparison.OrdinalIgnoreCase);
        Assert.Contains(NotFound, answer.Result, StringComparison.OrdinalIgnoreCase);

        // Simple filter: known user can see the memory
        answer = await KernelMemory.AskAsync("What is Orion?", filter: MemoryFilters.ByTag("user", "admin"), index: indexName).ConfigureAwait(false);
        Output.WriteLine(answer.Result);
        Assert.Contains(Found, answer.Result, StringComparison.OrdinalIgnoreCase);

        // Simple filter: known user can see the memory
        answer = await KernelMemory.AskAsync("What is Orion?", filter: MemoryFilters.ByTag("user", "owner"), index: indexName).ConfigureAwait(false);
        Output.WriteLine(answer.Result);
        Assert.Contains(Found, answer.Result, StringComparison.OrdinalIgnoreCase);

        // Simple filter: test AND logic with correct values
        answer = await KernelMemory.AskAsync("What is Orion?", filter: MemoryFilters.ByTag("type", "news").ByTag("user", "owner"), index: indexName).ConfigureAwait(false);
        Output.WriteLine(answer.Result);
        Assert.Contains(Found, answer.Result, StringComparison.OrdinalIgnoreCase);

        Output.WriteLine("Deleting memories extracted from the document");
        await KernelMemory.DeleteDocumentAsync(Id, indexName).ConfigureAwait(false);

        Output.WriteLine("Deleting index");
        await KernelMemory.DeleteIndexAsync(indexName).ConfigureAwait(false);
    }
    ...
    public async Task CanImportOneDocumentAndAskAsync()
    {
        ...
        var id = await KernelMemory.ImportDocumentAsync(
        ...
        Output.WriteLine($"Indexed document with id '{id}'.");

        // Waits for the documents to be saved
        var actualIndexName = IndexNameHelper.Convert(indexName, ElasticsearchConfig);
        //await this.Client.WaitForDocumentsAsync(actualIndexName, expectedDocuments: 2)
        //          .ConfigureAwait(false);

        // Asks a question on the data we just inserted
        MemoryAnswer? answer = await TryToGetTopAnswerAsync(indexName, "What can carbon bond to?")
            .ConfigureAwait(false);
        PrintAnswerOfDocument(answer, "doc001");
    }
    ...
    public async Task CanImportTwoDocumentsAndAskAsync()
    {
        ...
        docId = await KernelMemory.ImportDocumentAsync(
                new Document("doc002")
                    .AddFiles([
        ...
        docId = await KernelMemory.ImportDocumentAsync(new Document("doc003")
                    .AddFile(TestsHelper.NASANewsFileName)
                    .AddTag("user", "Taylor")
        ...
        var answer = await KernelMemory.AskAsync("What's E = m*c^2?", indexName)
            .ConfigureAwait(false);

        PrintAnswerOfDocument(answer, "doc001");

        // This should return a citation to doc002
        answer = await KernelMemory.AskAsync("What's Semantic Kernel?", indexName)
            .ConfigureAwait(false);

        PrintAnswerOfDocument(answer, "doc002");
    }
    ...
    private void PrintAnswerOfDocument(MemoryAnswer? answer, string expectedDocumentId)
    {
        ...
        Output.WriteLine($"Question: {answer.Question}");
        Output.WriteLine($"Answer: {answer.Result}");

        var foundDocumentReference = false;

        foreach (var citation in answer.RelevantSources)
        {
            Output.WriteLine($"  - {citation.SourceName}  - {citation.Link} [{citation.Partitions.First().LastUpdate:D}]");
    ...
    private async Task<MemoryAnswer?> TryToGetTopAnswerAsync(string indexName, string question)
    {
        ...
        for (int i = 0; i < 3; i++)
        {
            answer = await KernelMemory.AskAsync(
                    question,
                    indexName)
                .ConfigureAwait(false);

...
public class AzureAIContentSafetyModeration : IContentModeration
{
    ...
    {
        config.Validate();
        _log = (loggerFactory ?? DefaultLogger.Factory).CreateLogger<AzureAIContentSafetyModeration>();
        _globalSafetyThreshold = config.GlobalSafetyThreshold;
        _ignoredWords = config.IgnoredWords;
        ...
        switch (config.Auth)
        {
            case AzureAIContentSafetyConfig.AuthTypes.AzureIdentity:
                _client = new ContentSafetyClient(new Uri(config.Endpoint), new DefaultAzureCredential(), clientOptions);
                break;

            case AzureAIContentSafetyConfig.AuthTypes.APIKey:
                _client = new ContentSafetyClient(new Uri(config.Endpoint), new AzureKeyCredential(config.APIKey), clientOptions);
                break;

            case AzureAIContentSafetyConfig.AuthTypes.ManualTokenCredential:
                _client = new ContentSafetyClient(new Uri(config.Endpoint), config.GetTokenCredential(), clientOptions);
                break;
    ...
    {
        return IsSafeAsync(text, _globalSafetyThreshold, cancellationToken);
    }
    ...
    public async Task<bool> IsSafeAsync(string? text, double threshold, CancellationToken cancellationToken)
    {
        ...
        text = RemoveIgnoredWords(text);

        Response<AnalyzeTextResult>? result = await _client.AnalyzeTextAsync(text, cancellationToken).ConfigureAwait(false);

        bool isSafe = result.HasValue && result.Value.CategoriesAnalysis.All(x => x.Severity <= threshold);

        if (!isSafe)
        {
            IEnumerable<string> report = result.HasValue
                ? result.Value.CategoriesAnalysis.Select(x => $"{x.Category}: {x.Severity}")
                : [];
            _log.LogWarning("Unsafe content detected, report: {0}", string.Join("; ", report));
            _log.LogSensitive("Unsafe content: {0}", text);
        }
    ...
    {
        foreach (var word in _ignoredWords)
        {
